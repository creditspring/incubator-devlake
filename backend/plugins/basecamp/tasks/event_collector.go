...
const RAW_EVENT_TABLE = "basecamp_events"

var _ plugin.SubTaskEntryPoint = CollectEvents

var CollectEventsMeta = plugin.SubTaskMeta{
	Name:             "CollectEvents",
	EntryPoint:       CollectEvents,
	EnabledByDefault: true,
	Description:      "Collect events from todos in Basecamp account",
	DomainTypes:      []string{plugin.DOMAIN_TYPE_TICKET},
}

func CollectEvents(taskCtx plugin.SubTaskContext) errors.Error {
	...
	if len(todos) == 0 {
		taskCtx.GetLogger().Info("No todos found, skipping event collection")
		return nil
	}

	taskCtx.GetLogger().Info("Collecting events from %d todos", len(todos))
	...
	collector, err := api.NewApiCollector(api.ApiCollectorArgs{
		...
		UrlTemplate: fmt.Sprintf("%s/buckets/{{ .Input.ProjectId }}/recordings/{{ .Input.TodoId }}/events.json",
			data.AccountId),
		...
		ResponseParser: func(res *http.Response) ([]json.RawMessage, errors.Error) {
			...
			err := api.UnmarshalResponse(res, &events)
			if err != nil {
				return nil, err
			}
			return events, nil
